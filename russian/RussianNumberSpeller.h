...
public:
	...
	virtual void SpellHundreds (int nHundreds) const override
    {
        AppendHundreds (nHundreds);
    }

	virtual void SpellTens (int nTens) const override
    {
        AppendTens (nTens);
    }

	virtual void Spell1to19 (int n) const override
    {
...
public:
	...
	{
        auto lang = unique_ptr <ICaseSpeller> (CreateSpeller (sb, n, padeg, unit));
    ...
    virtual tstring GetUnit (size_t rank) const = 0;
	virtual const TCHAR * SpellZero (int padeg) const = 0;
	virtual tstring GenitivePlural (const tstring & unit) const = 0;

	tstring Ordinal (size_t exponent) const
	{
	...
	tstring OrdinalStem (size_t exponent) const
    {
        switch (exponent)
        {
	...
	virtual tstring GenitiveSingular (const tstring & unit) const = 0;
};
...
public:
	...
	virtual const TCHAR * Minus () const override { return _T("минус"); }

	virtual tstring GenitiveSingular (const tstring & unit) const override
	{
	...
	virtual tstring GenitivePlural (const tstring & unit) const override
	{
		PhraseWrapper phraseWrapper = ParseUnit(unit);
	...
	virtual const TCHAR * SpellZero (int padeg) const override
	{
	...
	virtual tstring GetUnit (size_t rank) const override
    {
	...
	static list <Ptr <const IPhrasePart> > MakeList (const Ptr <const IPhrasePart> & elem)
	{
	...
	Phrase* ParseUnit(const tstring & unit) const
	{
		...
		if (unit == _T("человек")) return new Phrase (MakeList (new SpecialUnit (manSing)), SingMasc, true, false, 
			new Phrase (MakeList (new SpecialUnit (manPlur)), Plural, true, false, nullptr));
		...
		if (unit == _T("год")) return new Phrase (MakeList (new SpecialUnit (yearSing)), SingMasc, false, false, 
			new Phrase (MakeList (new SpecialUnit (yearPlur)), Plural, false, false, nullptr));

		return morpher->Analyse (unit, static_cast<Attributes>(0));
	}

	virtual ICaseSpeller * CreateSpeller (StringBuilder & sb, int n, int padeg, tstring * unit) const override
	{
		PhraseWrapper phraseWrapper = ParseUnit(*unit);
