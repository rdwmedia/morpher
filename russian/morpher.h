...
public:
	...
	tstring DecoratedLemma () const
	{
		return ApplyTo (lemma);
	}

    DecoratedWord Clone (const tstring & newLemma) const
    {
        return DecoratedWord (newLemma, caps, sya, yer);
    }
};

static tstring Join (const vector <DecoratedWord> & v, const tstring & separator)
{
...
public:
	...
	virtual tstring GetFormByCase (Padeg padeg, bool animate) const override
    {
		return s;
	}
...
public:
    ...
    const list <Ptr <const IPhrasePart> > & Parts () const
    {
        return parts; 
    }
...
public:
	tstring GetForm (Padeg padeg) const
	{
        tstring s = _T("");
	...
	tstring PaucalForm () const
	{
        ...
        {
			Ptr <const IPhrasePart> p = (*singularIter)->PaulcalIsSingular () ? *singularIter : pluralPhrase ? *pluralIter : Ptr <const IPhrasePart> (nullptr);
			if (!p) return _T("");
...
public:
	...
	virtual const GramInfo * Nominative () const override { return this; } 
};
...
public:
	...
	virtual tstring operator [] (Padeg c) const override {return row->endings [c];}
	virtual bool AccAnimEqualsGen () const override {return row->accAnimEqualsGen;}
    virtual bool IsNotInflected () const override {return row == &Paradigms::Uninflected;}
	virtual EndingsType GetEndingsType () const override {return row->endingsType;}
};
...
public:
	_Indirect () {}
	virtual const GramInfo * Nominative () const override { return nullptr; } 
};
...
public:
	...
	FioDictionary ()
	{
		stringstream file (string (fioDict, fioDict_size));
...
class Dictionary 
{
	...
	private:
		...
		virtual tstring operator [] (Padeg c) const override
		{
			static const TCHAR * endings [] = {nullptr, nullptr, _T("ам"), nullptr, _T("ами"), _T("ах"), _T("ах")};
		...
		virtual bool AccAnimEqualsGen () const override {return true;}
        virtual bool IsNotInflected () const override {return false;}
		virtual EndingsType GetEndingsType () const override {return EndingsType_NounLike;}
	};
...
public:
	...
	{
		stringstream file (string (dict, dict_size));
		...
		for (size_t i=0; i < entryCount; ++i) // цикл по статьям
		{
			...
			const Info * gi = (gii == gis.size ()) ? static_cast<const Info *>(&Indirect) : & gis [gii];
	...
	{
		Entries::const_iterator entry0 = upper_bound (entries.begin (), entries.end (), Entry (lemma, nullptr), Less);
	...
	{
		Entries::const_iterator entry0 = lower_bound (entries.begin (), entries.end (), Entry (lemma, nullptr), Less);
...
class RussianDeclensor : public IMorpher // C#: Склонятель
{
	...
	public:
	    ...
	    virtual bool AppendTo (PhraseBuilder & phraseBuilder, list <tstring>::const_iterator & separator, bool plural) const override
		{
            ...
            for (size_t part=0; part < multiPartDecoratedWord.PartCount (); ++part)
            {
				...
				const ParameterlessParadigm & se = * static_cast<const ParameterlessParadigm *>(gramInfo[part]->GetEndings());
	...
	static bool AllWordsLowercase (const list <MultiPartDecoratedWord> & components)
	{
	...
	public:
		...
		virtual const tstring & Lemma () const override
		{
	...
	class SpecialDictionary // C#: СловарьОсобыхСлов
	{
		...
		public:
			...
			virtual tstring GetFormByCase (Padeg padeg, bool isAnimate) const override
		    {
	...
	public:
		...
		{
			{
				stringstream file (string (static_cast<char *>(specialDict), specialDict_size));
            ...
            {
				stringstream file (string (specialAdjDict, specialAdjDict_size));
	...
	private:
		...
		static vector <tstring> ReadForms (LemmaReader & lemmaReader)
        {
        ...
        static vector <tstring> RemoveYo (const vector <tstring> & forms)
        {
...
public:
	...
	virtual Phrase * Analyse (const tstring & s, Attributes attributes) const override
	{
	...
	Phrase * AnalyseInternal (const tstring & s, Attributes attributes) const
	{
        ...
        return new Phrase (phraseBuilder.Parts (), rodChislo, animate, prepositionNA, 
            !plural ? 0 : new Phrase (phraseBuilderPlural.Parts (), Plural, animate, prepositionNA, nullptr));
	}
};
