...
static Currencies BuildCurrencies ()
{
	...
	static Currency byn = {_T("белорусский рубль"), _T("копейка")};
	...
	ccys [_T("бел. руб")] = byn;
	ccys [_T("бел. руб.")] = byn;
	ccys [_T("белорусский рубль")] = byn;
	ccys [_T("белорусские рубли")] = byn;
...
CurrencySpellResult RussianCurrencySpeller::Spell2(const tstring& number, const tstring& unit, int padeg) const
{
...
static tstring FormatDoubleNicely(double d)
{
...
CurrencySpellResult RussianCurrencySpeller::Spell(double number, const tstring& unit, int padeg) const
{
	return Spell(FormatDoubleNicely(number), unit, padeg);
}

CurrencySpellResult RussianCurrencySpeller::Spell(const tstring& number, const tstring& unit, int padeg) const
{
