    ...
    int flags)
{
	...
	//printf ("utf8_to_wchar: start: insize=%d\n", insize);
	//fflush (stdout);

	if (in == NULL || insize == 0 || (outsize == 0 && out != NULL))
	{
		//puts ("utf8_to_wchar: invalid input");
		return (0);
	}
	...
	for (; left > 0; p += n, left -= n) {
		...
		if (out >= wlim)
		{
			//puts ("utf8_to_wchar: not enough space");
		...
		*out |= high << n_bits;
		...
		if (__wchar_forbitten(*out) != 0) {
			if ((flags & UTF8_IGNORE_ERROR) == 0)
			{
				//puts ("utf8_to_wchar: forbitten character");
		...
		//printf ("utf8_to_wchar: p=%d lim=%d p<lim=%d left=%d n=%d char=%c\n", p, lim, p < lim, left, n, *out);

		out++;
	}

	//printf ("utf8_to_wchar: total = %d\n", total);
